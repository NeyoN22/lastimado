...
if(!isset($_GET['id'])){
    ...
    redirect('admin/?page=booking');
}
$order = $conn->query("SELECT o.*,concat(c.firstname,' ',c.lastname) as client FROM `book_order` o inner join clients c on c.id = o.client_id where o.id = '{$_GET['id']}' ");
if($order->num_rows > 0){
    ...
    <div class="card-body">
        <div class="conitaner-fluid">
            ...
            <table class="table-striped table table-bordered">
                ...
                <tbody>
                    <?php 
                        $olist = $conn->query("SELECT book_order.*,p.title,p.author FROM booking_list book_order inner join products p on book_order.product_id = p.id where book_order.book_id = '{$id}' ");
...
<script>
    $(function(){
        $('#update_status').click(function(){
            uni_modal("Update Status", "./booking/book_update_status.php?oid=<?php echo $id ?>&status=<?php echo $status ?>")
        })
    })
</script>