...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">List of Booking Orders</h3>
        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT o.*,concat(c.firstname,' ',c.lastname) as client from `book_order` o inner join clients c on c.id = o.client_id order by unix_timestamp(o.date_created) desc ");
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
				                    <a class="dropdown-item" href="?page=booking/book_view_order&id=<?php echo $row['id'] ?>">View Order</a>
...
<script>
	$(document).ready(function(){
		...
		$('.pay_order').click(function(){
			_conf("Are you sure to mark this booking as paid?","pay_booking",[$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})
	function pay_order($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=pay_booking",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
